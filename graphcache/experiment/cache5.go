...
type PackMap struct {
	...
	workerWorkC      chan *bucket
	workerErrorC     chan error
	buckets          []*bucket
	allocatedBuckets []*bucket
	digester         *xxhash.XXHash64
}

func NewPackMap(numWorkers int) *PackMap {
	newMap := &PackMap{
		pendingChanges: 0,
		workerWG:       &sync.WaitGroup{},
		workerWorkC:    make(chan *bucket),
		workerErrorC:   make(chan error),
		buckets:        make([]*bucket, NumMapBuckets),
		digester:       xxhash.New64(),
	}

	newMap.startWorkers(numWorkers)
	return newMap
}

func (s *PackMap) startWorkers(numWorkers int) {
	for workerID := 0; workerID < numWorkers; workerID++ {
		go func(workerID int, frameBuffer []byte, writer *gzip.Writer) {
			for nextBucket := range s.workerWorkC {
...
func (s *PackMap) Compact() error {
	...
	for idx := 0; idx < len(s.allocatedBuckets); idx++ {
		s.workerWG.Add(1)
		s.workerWorkC <- s.allocatedBuckets[idx]
	}

	s.workerWG.Wait()

	select {
	case err := <-s.workerErrorC:
		workerErrors := []error{err}

		for done := false; !done; {
			select {
			case nextErr, ok := <-s.workerErrorC:
				if ok {
					workerErrors = append(workerErrors, nextErr)
				} else {
					done = true
				}
