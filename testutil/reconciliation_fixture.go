...
func NewDirectWriteScaleFixture(targets int) *opengraph.Graph {
	...
	fixture := &opengraph.Graph{
		Nodes: []opengraph.Node{
			{
				ID:         "write-root",
				Kinds:      []string{"WriteEndpoint"},
				Properties: map[string]any{"objectid": "write-root", "role": "root"},
			},
			{
				ID:         "write-survivor",
				Kinds:      []string{"WriteEndpoint"},
				Properties: map[string]any{"objectid": "write-survivor", "role": "survivor"},
			},
		},
		Edges: []opengraph.Edge{
			{
				StartID:    "write-root",
				EndID:      "write-survivor",
				Kind:       "WriteSurvivor",
			...
			},
			{
				...
				Properties: map[string]any{"deletebatch": false, "marker": "same-kind-survivor"},
			},
		},
	}
		...
		Nodes: []opengraph.Node{
			{
				ID:         "source",
				Kinds:      []string{"Source"},
				Properties: map[string]any{"objectid": "source"},
			},
			{
				ID:         "list-source-duplicate",
				Kinds:      []string{"Source"},
				Properties: map[string]any{"objectid": "list-source-duplicate"},
			},
			{
				ID:         "sink",
				Kinds:      []string{"Destination"},
				Properties: map[string]any{"objectid": "sink"},
			},
			{
				ID:         "inbound-target",
				Kinds:      []string{"ADEntity", "Group"},
				Properties: map[string]any{"objectid": "rec-in"},
			},
			{
				ID:         "outbound-target",
				Kinds:      []string{"ADEntity", "Computer"},
				Properties: map[string]any{"objectid": "rec-out"},
			},
			{
				ID:         "list-target",
				Kinds:      []string{"ADEntity", "User"},
				Properties: map[string]any{"objectid": "rec-list"},
			},
			{
				ID:         "template",
				Kinds:      []string{"CertTemplate"},
				Properties: map[string]any{"objectid": "template"},
			},
			{
				ID:         "agent",
				Kinds:      []string{"ADEntity", "User"},
				Properties: map[string]any{"objectid": "agent"},
			},
			{
				ID:         "agent-duplicate",
				Kinds:      []string{"ADEntity", "User"},
				Properties: map[string]any{"objectid": "agent-duplicate"},
			},
			{
				ID:         "delete-target",
				Kinds:      []string{"ADEntity", "Group"},
				Properties: map[string]any{"objectid": "delete-target"},
			},
			{
				ID:         "survivor",
				Kinds:      []string{"ADEntity", "User"},
				Properties: map[string]any{"objectid": "survivor"},
			},
		},
		Edges: []opengraph.Edge{
			{
				StartID:    "source",
				EndID:      "inbound-target",
				Kind:       "RecKind01",
				Properties: map[string]any{"marker": "rec-01-a"},
			},
			{
				StartID:    "source",
				EndID:      "inbound-target",
				Kind:       "RecKind30",
				Properties: map[string]any{"marker": "rec-01-b"},
			},
			{
				StartID:    "outbound-target",
				EndID:      "sink",
				Kind:       "RecKind01",
				Properties: map[string]any{"marker": "rec-02-a"},
			},
			{
				StartID:    "outbound-target",
				EndID:      "sink",
				Kind:       "RecKind30",
				Properties: map[string]any{"marker": "rec-02-b"},
			},
			{
				StartID:    "source",
				EndID:      "list-target",
				Kind:       "ADReconcile",
				Properties: map[string]any{"marker": "rec-04-a"},
			},
			{
				StartID:    "list-source-duplicate",
				EndID:      "list-target",
				Kind:       "ADReconcile",
				Properties: map[string]any{"marker": "rec-04-b"},
			},
			{
				StartID:    "agent",
				EndID:      "template",
				Kind:       "DelegatedEnrollmentAgent",
				Properties: map[string]any{"marker": "rec-06-a"},
			},
			{
				StartID:    "agent-duplicate",
				EndID:      "template",
				Kind:       "DelegatedEnrollmentAgent",
				Properties: map[string]any{"marker": "rec-06-b"},
			},
			{
				StartID:    "source",
				EndID:      "survivor",
		...
		Nodes: []opengraph.Node{
			{
				ID:         "trust-early",
				Kinds:      []string{"Domain"},
				Properties: map[string]any{"lastcollected": "2026-01-02T00:00:00Z"},
			},
			{
				ID:         "trust-late-a",
				Kinds:      []string{"Domain"},
				Properties: map[string]any{"lastcollected": "2026-01-04T00:00:00Z"},
			},
			{
				ID:         "trust-late-b",
				Kinds:      []string{"Domain"},
			...
			},
			{
				ID:         "trust-equal-a",
				Kinds:      []string{"Domain"},
				Properties: map[string]any{"lastcollected": "2026-01-03T00:00:00Z"},
			},
			{
				ID:         "trust-equal-b",
				Kinds:      []string{"Domain"},
			...
			{
				ID:         "prune-missing",
				Kinds:      []string{"PruneCandidate"},
				Properties: map[string]any{"name": "missing"},
			},
			{
				ID:         "prune-null",
				Kinds:      []string{"PruneCandidate"},
				Properties: map[string]any{"name": "null", "lastseen": nil},
			},
			{
				ID:         "prune-protected",
				Kinds:      []string{"PruneCandidate", "Domain"},
				Properties: map[string]any{"name": "protected", "lastseen": "2026-01-02T00:00:00Z"},
			},
			{
				ID:         "orphan-missing",
				Kinds:      []string{"PruneCandidate"},
			...
			},
			{
				ID:         "orphan-named",
				Kinds:      []string{"PruneCandidate"},
				Properties: map[string]any{"name": "named", "objectid": "S-1-5-102"},
			},
			{
			...
			{
				ID:         "prune-batch-high",
				Kinds:      []string{"PruneBatchNode"},
				Properties: map[string]any{"remove": true},
			},
			{
				ID:         "prune-batch-survivor",
				Kinds:      []string{"PruneBatchNode"},
				Properties: map[string]any{"remove": false},
			},
		},
		Edges: []opengraph.Edge{
			{
				StartID:    "trust-equal-a",
				EndID:      "trust-equal-b",
				Kind:       "SameForestTrust",
				Properties: map[string]any{"lastseen": "2026-01-03T00:00:00Z", "marker": "same-equal"},
			},
			{
				StartID:    "trust-equal-a",
				EndID:      "trust-equal-b",
				Kind:       "CrossForestTrust",
				Properties: map[string]any{"lastseen": "2026-01-03T00:00:00Z", "marker": "cross-equal"},
			},
			{
				StartID:    "trust-late-a",
				EndID:      "trust-late-b",
				Kind:       "SameForestTrust",
				Properties: map[string]any{"lastseen": "2026-01-05T00:00:00Z", "marker": "same-new"},
			},
			{
				StartID:    "trust-late-a",
				EndID:      "trust-late-b",
				Kind:       "CrossForestTrust",
				Properties: map[string]any{"lastseen": "2026-01-05T00:00:00Z", "marker": "cross-new"},
			},
			{
				StartID:    "trust-late-a",
				EndID:      "trust-late-b",
			...
			},
			{
				...
				EndID:      "prune-b",
				Kind:       "MetaIncludes",
				Properties: map[string]any{"lastseen": "2026-01-02T00:00:00Z", "marker": "protected-meta-includes"},
			},
		},
	}
	...
	for idx := range fanout {
		...
		fixture.Nodes = append(fixture.Nodes,
			...
			opengraph.Node{
				ID:         oldNodeID,
				Kinds:      []string{"PruneCandidate"},
				Properties: map[string]any{"name": oldNodeID, "lastseen": "2026-01-02T00:00:00Z"},
			},
			opengraph.Node{
				ID:         newNodeID,
				Kinds:      []string{"PruneCandidate"},
				Properties: map[string]any{"name": newNodeID, "lastseen": "2026-01-04T00:00:00Z"},
			},
			opengraph.Node{
				ID:         orphanNodeID,
				Kinds:      []string{"PruneCandidate"},
				Properties: map[string]any{"objectid": "S-1-5-" + suffix},
			},
			opengraph.Node{
				ID:         batchNodeID,
				Kinds:      []string{"PruneBatchNode"},
				Properties: map[string]any{"remove": idx%2 == 0},
			},
			opengraph.Node{
				ID:         neighborID,
			...
			opengraph.Node{
				ID:         candidateOldTargetID,
				Kinds:      []string{"PruneEndpoint"},
				Properties: map[string]any{"name": candidateOldTargetID},
			},
			opengraph.Node{
				ID:         candidateNewTargetID,
				Kinds:      []string{"PruneEndpoint"},
				Properties: map[string]any{"name": candidateNewTargetID},
			},
			opengraph.Node{
				ID:         sessionMissingTargetID,
				Kinds:      []string{"PruneEndpoint"},
				Properties: map[string]any{"name": sessionMissingTargetID},
			},
			opengraph.Node{
				ID:         sessionOldTargetID,
				Kinds:      []string{"PruneEndpoint"},
				Properties: map[string]any{"name": sessionOldTargetID},
			},
			opengraph.Node{
				ID:         sessionEqualTargetID,
				Kinds:      []string{"PruneEndpoint"},
				Properties: map[string]any{"name": sessionEqualTargetID},
			},
			opengraph.Node{
				ID:         batchEdgeTargetID,
				Kinds:      []string{"PruneEndpoint"},
				Properties: map[string]any{"name": batchEdgeTargetID},
			},
		)
		...
		fixture.Edges = append(fixture.Edges,
			opengraph.Edge{
				...
				EndID:      trustEarlyID,
				Kind:       "SameForestTrust",
				Properties: map[string]any{"lastseen": "2026-01-03T00:00:00Z", "marker": "same-old-" + suffix},
			},
			opengraph.Edge{
				StartID:    "trust-late-a",
				EndID:      trustEarlyID,
				Kind:       "CrossForestTrust",
				Properties: map[string]any{"lastseen": "2026-01-03T00:00:00Z", "marker": "cross-old-" + suffix},
			},
			opengraph.Edge{
				StartID:    "prune-a",
				EndID:      candidateOldTargetID,
				Kind:       "CandidateRel",
				Properties: map[string]any{"lastseen": "2026-01-02T00:00:00Z", "marker": "candidate-old-" + suffix},
			},
			opengraph.Edge{
				StartID:    "prune-a",
				EndID:      candidateNewTargetID,
				Kind:       "CandidateRel",
				Properties: map[string]any{"lastseen": "2026-01-04T00:00:00Z", "marker": "candidate-new-" + suffix},
			},
			opengraph.Edge{
				StartID:    "prune-a",
				EndID:      sessionMissingTargetID,
				Kind:       "HasSession",
				Properties: map[string]any{"marker": "session-missing-" + suffix},
			},
			opengraph.Edge{
				StartID:    "prune-a",
				EndID:      sessionOldTargetID,
				Kind:       "HasSession",
				Properties: map[string]any{"lastseen": "2026-01-02T00:00:00Z", "marker": "session-old-" + suffix},
			},
			opengraph.Edge{
				StartID:    "prune-a",
				EndID:      sessionEqualTargetID,
				Kind:       "HasSession",
				Properties: map[string]any{"lastseen": "2026-01-03T00:00:00Z", "marker": "session-equal-" + suffix},
			},
			opengraph.Edge{
				StartID:    "prune-a",
				EndID:      batchEdgeTargetID,
				Kind:       "PruneBatch",
				Properties: map[string]any{"remove": true, "marker": "batch-" + suffix},
			},
			opengraph.Edge{
				StartID:    "prune-batch-high",
				EndID:      neighborID,
				Kind:       "PruneIncident",
				Properties: map[string]any{"marker": "incident-" + suffix},
			},
		)
	}
...
func NewHopScaleFixture(fanout int) *opengraph.Graph {
	...
	fixture := &opengraph.Graph{
		Nodes: []opengraph.Node{
			{
				ID:         "hop-out-root",
				Kinds:      []string{"HopAnchor"},
				Properties: map[string]any{"name": "hop-out-root"},
			},
			{
				ID:         "hop-in-root",
				Kinds:      []string{"HopAnchor"},
				Properties: map[string]any{"name": "hop-in-root"},
			},
			{
				ID:         "hop-kind-root",
				Kinds:      []string{"HopAnchor"},
				Properties: map[string]any{"name": "hop-kind-root"},
			},
			{
				ID:         "hop-decoy-root",
				Kinds:      []string{"HopAnchor"},
				Properties: map[string]any{"name": "hop-decoy-root"},
			},
		},
	}
	...
	for idx := range fanout {
		...
		fixture.Nodes = append(fixture.Nodes,
			...
			opengraph.Node{
				ID:         sourceID,
				Kinds:      []string{"HopSource"},
				Properties: map[string]any{"name": sourceID},
			},
		)
		fixture.Edges = append(fixture.Edges,
			opengraph.Edge{
				...
				EndID:      peerID,
				Kind:       "HopKind01",
				Properties: map[string]any{"marker": "out-" + suffix},
			},
			opengraph.Edge{
				StartID:    sourceID,
				EndID:      "hop-in-root",
				Kind:       "HopKind01",
				Properties: map[string]any{"marker": "in-" + suffix},
			},
			opengraph.Edge{
				StartID:    "hop-kind-root",
				EndID:      peerID,
				Kind:       fmt.Sprintf("HopKind%02d", idx%30+1),
				Properties: map[string]any{"marker": "kind-" + suffix},
			},
			opengraph.Edge{
				StartID:    "hop-out-root",
				EndID:      peerID,
				Kind:       "HopTypedEdge",
				Properties: map[string]any{"marker": "typed-" + suffix},
			},
			opengraph.Edge{
				StartID:    "hop-out-root",
				EndID:      peerID,
				Kind:       "HopNestedEdge",
				Properties: map[string]any{"marker": "nested-" + suffix},
			},
		)
	}
	...
	for _, startID := range setStarts {
		fixture.Nodes = append(fixture.Nodes, opengraph.Node{
			ID:         startID,
			Kinds:      []string{"HopSetStart"},
			Properties: map[string]any{"name": startID},
		})
	}
	for _, endID := range setEnds {
		fixture.Nodes = append(fixture.Nodes, opengraph.Node{
			ID:         endID,
			Kinds:      []string{"HopSetEnd"},
			Properties: map[string]any{"name": endID},
		})
	}
	for _, startID := range setStarts {
		for _, endID := range setEnds {
	...
	fixture.Edges = append(fixture.Edges,
		opengraph.Edge{
			StartID:    "hop-decoy-root",
			EndID:      "hop-peer-0000",
			Kind:       "HopTypedEdge",
			Properties: map[string]any{"marker": "wrong-root"},
		},
		opengraph.Edge{
			StartID:    "hop-peer-0000",
			EndID:      "hop-out-root",
			Kind:       "HopTypedEdge",
			Properties: map[string]any{"marker": "wrong-direction"},
		},
		opengraph.Edge{
			StartID:    setStarts[0],
			EndID:      setEnds[0],
			Kind:       "HopWrongSetEdge",
			Properties: map[string]any{"marker": "wrong-set-kind"},
		},
		opengraph.Edge{
			StartID:    setEnds[0],
			EndID:      setStarts[0],
			Kind:       "HopSetEdge",
			Properties: map[string]any{"marker": "wrong-set-direction"},
		},
	)
	return fixture
}
...
func NewScanLookupScaleFixture(fanout int) *opengraph.Graph {
		...
		Nodes: []opengraph.Node{
			{
				ID:         "scan-base-root",
				Kinds:      []string{"ADBase"},
				Properties: map[string]any{"name": "scan-base-root"},
			},
			{
				ID:         "scan-tracker-root",
				Kinds:      []string{"Plain"},
				Properties: map[string]any{"name": "scan-tracker-root"},
			},
			{
				ID:         "scan-nine-kind-target",
				Kinds:      []string{"Computer"},
				Properties: map[string]any{"name": "scan-nine-kind-target"},
			},
			{
				ID:         "scan-local-target",
				Kinds:      []string{"Computer"},
				Properties: map[string]any{"name": "scan-local-target"},
			},
			{
				ID:         "lookup-tenant",
				Kinds:      []string{"Tenant"},
				Properties: map[string]any{"name": "lookup-tenant", "objectid": "tenant-scale"},
			},
			// The extra isolated labels make negative Meta/MetaDetail predicates
			// translatable without changing any fixture cardinality.
			{
				ID:         "lookup-local-target",
				Kinds:      []string{"Computer", "Meta", "MetaDetail"},
				Properties: map[string]any{"name": "lookup-local-target"},
			},
		},
	}
	...
	for idx := range fanout {
		...
		fixture.Nodes = append(fixture.Nodes,
			opengraph.Node{
				ID:         scanEndID,
				Kinds:      []string{"AZBase", "Plain"},
				Properties: map[string]any{"name": scanEndID},
			},
			opengraph.Node{
				ID:         scanEntityID,
				Kinds:      entityKinds,
				Properties: map[string]any{"name": scanEntityID},
			},
			opengraph.Node{
				ID:         lookupObjectID,
				Kinds:      []string{"Computer"},
				Properties: map[string]any{"name": lookupObjectID, "objectid": "S-1-5-21-scale", "enabled": true},
			},
			opengraph.Node{
				ID:         lookupStringID,
				Kinds:      []string{"Group", "Entity"},
				Properties: map[string]any{"name": lookupName, "objectid": "S-1-5-21" + lookupObjectSuffix, "domainsid": "S-1-5-21"},
			},
			opengraph.Node{
				ID:         lookupLocalID,
				Kinds:      []string{"LocalGroup", "Entity"},
				Properties: map[string]any{"name": lookupLocalID, "objectid": "S-1-5-21-555"},
			},
			opengraph.Node{
				ID:         ntlmID,
				Kinds:      []string{"Computer"},
				Properties: map[string]any{"name": ntlmID, "domainsid": "S-1-5-21", "isdc": true, "ldapavailable": true, "ldapsigning": false},
			},
		)
		...
		fixture.Edges = append(fixture.Edges,
			opengraph.Edge{
				StartID:    "scan-base-root",
				EndID:      scanEndID,
				Kind:       "ScanPostProcessed",
				Properties: map[string]any{"marker": "post-" + suffix},
			},
			opengraph.Edge{
				StartID:    "scan-tracker-root",
				EndID:      scanEndID,
				Kind:       "TrackerA",
				Properties: map[string]any{"marker": "tracker-a-" + suffix},
			},
			opengraph.Edge{
				StartID:    "scan-tracker-root",
				EndID:      scanEndID,
				Kind:       "TrackerB",
				Properties: map[string]any{"marker": "tracker-b-" + suffix},
			},
			opengraph.Edge{
				StartID:    "scan-tracker-root",
				EndID:      scanEndID,
			...
			opengraph.Edge{
				StartID:    scanEntityID,
				EndID:      scanEndID,
				Kind:       "OwnsRaw",
				Properties: map[string]any{"marker": "owns-" + suffix},
			},
			opengraph.Edge{
				StartID:    scanEntityID,
				EndID:      "scan-nine-kind-target",
				Kind:       fmt.Sprintf("ScanEdge%02d", idx%9+1),
				Properties: map[string]any{"marker": "scan-" + suffix},
			},
			opengraph.Edge{
				StartID:    scanEntityID,
				EndID:      "scan-local-target",
				Kind:       "LocalToComputer",
				Properties: map[string]any{"marker": "scan-local-" + suffix},
			},
			opengraph.Edge{
				StartID:    scanEntityID,
				EndID:      scanEndID,
				Kind:       "MemberOf",
				Properties: map[string]any{"marker": "member-" + suffix},
			},
			opengraph.Edge{
				StartID:    scanEntityID,
				EndID:      scanEndID,
				Kind:       "MemberOfLocalGroup",
				Properties: map[string]any{"marker": "member-local-" + suffix},
			},
			opengraph.Edge{
				StartID:    scanEntityID,
				EndID:      victimID,
				Kind:       escalationKinds[idx%len(escalationKinds)],
				Properties: map[string]any{"marker": "esc-" + suffix},
			},
			opengraph.Edge{
				StartID:    lookupLocalID,
				EndID:      "lookup-local-target",
				Kind:       "LocalToComputer",
				Properties: map[string]any{"marker": "lookup-local-" + suffix},
			},
		)
	}
	...
	for idx, victimID := range victimIDs {
		...
		fixture.Nodes = append(fixture.Nodes, opengraph.Node{
			ID:         victimID,
			Kinds:      victimKinds,
			Properties: map[string]any{"name": victimID},
		})
	}
	...
	for idx, roleID := range FixtureNames("lookup-role", 1_000) {
		...
		fixture.Nodes = append(fixture.Nodes, opengraph.Node{
			ID:         roleID,
			Kinds:      []string{"AZRole"},
			Properties: map[string]any{"name": roleID, "roletemplateid": roleTemplateID, "enabled": true},
		})
		fixture.Edges = append(fixture.Edges, opengraph.Edge{
			StartID:    "lookup-tenant",
			EndID:      roleID,
			Kind:       "Contains",
			Properties: map[string]any{"marker": roleID},
		})
	}
