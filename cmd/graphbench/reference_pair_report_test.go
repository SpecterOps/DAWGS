...
func TestBuildReferencePairReportComparesExactMatchedArms(t *testing.T) {
	...
	for round := 1; round <= 10; round++ {
		...
		record := CaseResult{
			...
			RowCount: 1, ObservedRows: []string{"[2]"}, Environment: &RunEnvironment{Round: round, WarmupIterations: 20},
			PostgresReferences: []PostgresReferenceResult{
				{Name: "s3", Architecture: "SP-S3-U-D", FullComparator: true, SemanticValidation: "exact_public_observation", RowCount: 1, ObservedRows: []string{"[2]"}, MeasurementOrder: baselineOrder, Stats: DurationStats{WarmupIterations: 20}},
				{Name: "s1", Architecture: "SP-S1", FullComparator: true, SemanticValidation: "exact_public_observation", RowCount: 1, ObservedRows: []string{"[2]"}, MeasurementOrder: candidateOrder, Stats: DurationStats{WarmupIterations: 20}},
			},
		}
	...
	report, err := buildReferencePairReport(records, ReferencePairOptions{Seed: 1, Confidence: 0.975, BootstrapCount: 100, BaselineName: "s3", CandidateName: "s1"})
...
func TestBuildReferencePairReportComparesValidatedHydrationBoundaries(t *testing.T) {
	...
	for round := 1; round <= 10; round++ {
		...
		record := CaseResult{
			Dataset: "fixture", Name: "path", ExecutionMode: ModePostgresSQL, Status: StatusOK,
			RowCount: 1, ObservedRows: []string{"[path]"}, Environment: &RunEnvironment{Round: round, WarmupIterations: 20},
			PostgresReferences: []PostgresReferenceResult{
				{Name: "m0", Architecture: "MAT-M0", Boundary: "edge IDs", SemanticValidation: "precomputed_exact_path_inputs", RowCount: 1, ObservedRows: []string{"[path]"}, MeasurementOrder: baselineOrder, Stats: DurationStats{WarmupIterations: 20}},
				{Name: "m1", Architecture: "MAT-M1", Boundary: "node and edge IDs", SemanticValidation: "precomputed_exact_path_inputs", RowCount: 1, ObservedRows: []string{"[path]"}, MeasurementOrder: candidateOrder, Stats: DurationStats{WarmupIterations: 20}},
			},
		}
		for iteration := 1; iteration <= 50; iteration++ {
			record.PostgresReferences[0].Stats.Samples = append(record.PostgresReferences[0].Stats.Samples, LatencySample{Round: round, Iteration: iteration, Classification: "warm", Duration: time.Millisecond})
	...
	report, err := buildReferencePairReport(records, ReferencePairOptions{Seed: 1, Confidence: 0.975, BootstrapCount: 100, BaselineName: "m0", CandidateName: "m1"})
	require.NoError(t, err)
	require.Len(t, report.Cases, 1)
...
func TestBuildReferencePairReportRejectsMixedExactBoundaries(t *testing.T) {
	record := CaseResult{
		Dataset: "fixture", Name: "path", ExecutionMode: ModePostgresSQL, Status: StatusOK,
		RowCount: 1, ObservedRows: []string{"[path]"}, Environment: &RunEnvironment{Round: 1, WarmupIterations: 20},
		PostgresReferences: []PostgresReferenceResult{
			{Name: "full", FullComparator: true, SemanticValidation: "exact_public_observation", RowCount: 1, ObservedRows: []string{"[path]"}, MeasurementOrder: 2, Stats: DurationStats{WarmupIterations: 20}},
			{Name: "hydration", SemanticValidation: "precomputed_exact_path_inputs", RowCount: 1, ObservedRows: []string{"[path]"}, MeasurementOrder: 3, Stats: DurationStats{WarmupIterations: 20}},
		},
	}

	_, err := buildReferencePairReport([]CaseResult{record}, ReferencePairOptions{Seed: 1, Confidence: 0.975, BaselineName: "full", CandidateName: "hydration"})
...
func TestBuildReferencePairReportSupportsLabeledOrderedIDDiscovery(t *testing.T) {
	...
	for round := 1; round <= 5; round++ {
		...
		for iteration := 1; iteration <= 10; iteration++ {
			record.PostgresReferences[0].Stats.Samples = append(record.PostgresReferences[0].Stats.Samples, LatencySample{Round: round, Iteration: iteration, Classification: "warm", Duration: 2 * time.Millisecond})
			record.PostgresReferences[1].Stats.Samples = append(record.PostgresReferences[1].Stats.Samples, LatencySample{Round: round, Iteration: iteration, Classification: "warm", Duration: time.Millisecond})
		}
		records = append(records, record)
	}
