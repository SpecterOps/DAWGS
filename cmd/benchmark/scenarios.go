...
var defaultDatasets = []string{"base", traversalShapesDataset, "adcs_fanout"}
...
func expectScenarioRows(s Scenario, rows int64) Scenario {
	s.ExpectedRows = expectRows(rows)
	return s
}

func countScenario(section, dataset, label string, query func(tx graph.Transaction) (int64, error), expectedRows int64) Scenario {
	return Scenario{
		Section:      section,
		Dataset:      dataset,
		Label:        label,
		ExpectedRows: expectRows(expectedRows),
...
func baseScenarios(idMap opengraph.IDMap) []Scenario {
	ds := "base"
	return []Scenario{
		countScenario("Match Nodes", ds, ds, countNodes, 3),
		countScenario("Match Edges", ds, ds, countEdges, 2),
...
func traversalShapesScenarios(idMap opengraph.IDMap) []Scenario {
	...
	return []Scenario{
		countScenario("Match Nodes", ds, ds, countNodes, 45),
		countScenario("Match Edges", ds, ds, countEdges, 41),
