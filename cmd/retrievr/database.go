...
func resolveGraphTargets(ctx context.Context, db graph.Database, driverName string, requested []string, allGraphs bool) ([]graphTarget, error) {
...
func discoverPostgresGraphs(ctx context.Context, db graph.Database) ([]graphTarget, error) {
	const graphQuery = `
select
  g.name,
