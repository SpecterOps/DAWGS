...
func testPreparedWarmups(t *testing.T, statements ...string) []preparedStatementWarmup {
	t.Helper()
...
func TestStatementWarmupPolicyGenerationsAndSnapshots(t *testing.T) {
	policy := &statementWarmupPolicy{}
	policy.replace(testPreparedWarmups(t, "select 1"))

	generation, snapshot := policy.snapshot()
	require.Equal(t, uint64(1), generation)
	require.Len(t, snapshot, 1)
	snapshot[0].sql = "select changed"

	_, current := policy.snapshot()
	require.Equal(t, "select 1", current[0].sql)

	policy.replace(testPreparedWarmups(t, "select 2"))
	generation, current = policy.snapshot()
	require.Equal(t, uint64(2), generation)
	require.Equal(t, "select 2", current[0].sql)
...
func TestWarmCurrentStatementPolicyRetriesSupersededGeneration(t *testing.T) {
	policy := &statementWarmupPolicy{}
	policy.replace(testPreparedWarmups(t, "select 1"))
	firstWarmStarted := make(chan struct{})
	allowFirstWarm := make(chan struct{})
	var once sync.Once
	var warmed []string

	done := make(chan error, 1)
	go func() {
		done <- warmCurrentStatementPolicy(policy, func(statements []preparedStatementWarmup) error {
			if len(statements) > 0 {
				warmed = append(warmed, statements[0].sql)
			}
			once.Do(func() {
	...
	<-firstWarmStarted
	policy.replace(testPreparedWarmups(t, "select 2"))
	close(allowFirstWarm)
	require.NoError(t, <-done)
	require.Equal(t, []string{"select 1", "select 2"}, warmed)
}

func TestWarmCurrentStatementPolicyObservesClear(t *testing.T) {
	policy := &statementWarmupPolicy{}
	policy.replace(testPreparedWarmups(t, "select 1"))
	firstWarmStarted := make(chan struct{})
	allowFirstWarm := make(chan struct{})
	var once sync.Once
	var warmSetSizes []int

	done := make(chan error, 1)
	go func() {
		done <- warmCurrentStatementPolicy(policy, func(statements []preparedStatementWarmup) error {
			warmSetSizes = append(warmSetSizes, len(statements))
			once.Do(func() {
				close(firstWarmStarted)
				<-allowFirstWarm
			})
	...
	close(allowFirstWarm)
	require.NoError(t, <-done)
	require.Equal(t, []int{1, 0}, warmSetSizes)
}
