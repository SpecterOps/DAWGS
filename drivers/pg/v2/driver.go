...
type Driver struct {
	// delegate supplies the stable PostgreSQL implementation.
	delegate *pg.Driver

	// pool owns the physical connections and their cache provider.
	pool *Pool

	// closeOnce makes delegate and provider teardown idempotent.
	closeOnce sync.Once

	// closeErr preserves the first delegate-close result for later callers.
	closeErr error
}

// The graph.Database assertion ensures Driver preserves the public database contract.
