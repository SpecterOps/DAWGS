...
type sharedTemplateCache struct {
	// lock serializes shared template lookups, insertion, and statistics.
	lock sync.Mutex

	// capacity bounds retained immutable shortest-path templates.
	capacity int

	// sieve stores the shared immutable templates by translation key.
	sieve dawgscache.Cache[translationKey, translationEntry]

	// stats records query-text-free shared-cache activity.
	stats SharedTemplateStats
}

// newSharedTemplateCache creates a bounded shared tier when capacity is positive.
...
// get returns a retained template and records the lookup outcome.
...
// put retains a new immutable template unless the bounded tier already has it.
...
// snapshot returns a consistent query-text-free view of shared-cache activity.
