...
type CypherTranslationCache interface {
	// TranslateWithPolicy returns SQL generated or reused for the exact query, parameter shape, and policy.
	TranslateWithPolicy(
		query string,
		graphID int32,
...
type CypherTranslationCacheProvider interface {
	// CacheForConnection returns the cache owned by conn, or nil to bypass caching.
...
type cypherTranslationCache struct {
	...
	stats CypherTranslationCacheStats
}

// The CypherTranslationCache assertion ensures cypherTranslationCache satisfies the cache contract.
...
type sharedCypherTranslationCacheProvider struct {
	// cache is the driver-wide cache selected for every physical connection.
...
// The CypherTranslationCacheProvider assertion preserves v1's cache provider contract.
var _ CypherTranslationCacheProvider = (*sharedCypherTranslationCacheProvider)(nil)

// CypherTranslationCacheStats is a query-text-free snapshot of translation cache activity and occupancy.
type CypherTranslationCacheStats struct {
...
// cacheableCypherTranslation reports whether every translated parameter can be rebound from a current caller parameter.
func cacheableCypherTranslation(result translate.Result, parameters map[string]any) bool {
...
func (s *cypherTranslationCache) TranslateWithPolicy(query string, graphID int32, parameters map[string]any, policyIdentity string, build func() (translate.Result, string, error)) (string, map[string]any, error) {
	...
	cacheable := err == nil && cacheableCypherTranslation(result, parameters)
...
func (s *cypherTranslationCache) Stats() CypherTranslationCacheStats {
	if s == nil {
