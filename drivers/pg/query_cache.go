...
type cypherParseCacheShard struct {
	// lock serializes access to this shard's LRU, pending parses, and counters.
	lock sync.Mutex

	// capacity is the maximum number of completed ASTs this shard may retain.
	capacity int

	// entries maps normalized query text to its LRU entry.
	entries map[string]*list.Element

	// lru orders completed entries from most to least recently used.
	lru *list.List

	// pending coalesces callers parsing the same normalized query.
	pending map[string]*cypherParseCall

	// stats holds this shard's counters while lock is held.
...
// parseCacheShardCount chooses a bounded number of locks for an aggregate cache capacity.
...
// shardForQuery returns the deterministic shard that owns query's cache entry.
...
// parseCypher parses query in a fresh frontend context for cache misses and bypasses.
