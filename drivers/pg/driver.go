...
func (s *Driver) OptimizeStorage(ctx context.Context) error {
	...
	// partitioned parents hold no heap rows, so dead/live tuple stats relevant
	// to vacuum decisions come from the leaf partitions.
	const statsQuery = `
		SELECT
	...
	for _, table := range []string{"node", "edge"} {
		var dead, live int64
		if err := conn.QueryRow(ctx, statsQuery, table).Scan(&dead, &live); err != nil {
