...
type TraversalPolicy struct {
	// Generation distinguishes policy revisions in cache and rollout identities.
	Generation uint64 `json:"generation"`

	// PromotionManifestSHA256 verifies the immutable promotion manifest supplied below.
	...
	// QuerySHA256Allowlist is the exact set of query digests authorized by the manifest.
	QuerySHA256Allowlist []string `json:"query_sha256_allowlist"`

	// ShortestPathExecutor selects the manifest-backed shortest-path candidate.
	ShortestPathExecutor optimize.ShortestPathExecutor `json:"shortest_path_executor,omitempty"`

	// EnableExpansionOrientation enables the manifest-backed expansion orientation candidate.
	...
	// DisableEndpointSeededReverse immediately disables the endpoint-seeded reverse candidate.
	DisableEndpointSeededReverse bool `json:"disable_endpoint_seeded_reverse,omitempty"`

	// DisableInlineASPDAG immediately disables the inline ASP DAG candidate.
	DisableInlineASPDAG bool `json:"disable_inline_asp_dag,omitempty"`

	// DisableInlineSPWitness immediately disables the inline shortest-path witness candidate.
	...
	// compiledManifest is the decoded manifest used during translation and selection.
	compiledManifest traversalPromotionManifest

	// compiledBuckets maps every authorized query digest to its manifest bucket.
	compiledBuckets map[string]traversalPromotionBucket

	// compiledIdentity keys translations generated under this immutable policy snapshot.
...
// productionOptionsForShape derives options and an authorization bucket for query and shape.
...
// structuralSQLTemplateSHA256 returns bucket's required template digest under manifest.
...
// traversalPromotionBucket binds the workload, shape, and SQL contract that authorizes one candidate.
type traversalPromotionBucket struct {
	// Name identifies the qualified workload bucket.
	Name string `json:"name,omitempty"`
	// QuerySHA256 contains the exact query digests admitted to this bucket.
	...
	// ObservationMode identifies whether the traversal observes a distance, one path, or all paths.
	...
	// RelationshipKindCount is the number of relationship kinds constrained by the bucket.
	RelationshipKindCount int `json:"relationship_kind_count,omitempty"`
	// UntypedRelationship reports whether the bucket permits any relationship kind.
...
// traversalPromotionEvidence identifies an independently verifiable promotion-evidence artifact.
type traversalPromotionEvidence struct {
	// Path identifies the evidence document relative to its manifest.
	Path string `json:"path,omitempty"`
	// SHA256 verifies the evidence artifact's exact content.
...
type traversalPromotionManifest struct {
	// Version identifies the immutable manifest schema and routing protocol.
	Version int `json:"version"`
	// Candidate identifies the execution strategy being evaluated or authorized.
	Candidate string `json:"candidate"`
	// SelectorVersion identifies the candidate-selection protocol.
	SelectorVersion string `json:"selector_version"`
	// ExecutionBoundary identifies where the candidate and fallback are coordinated.
	ExecutionBoundary string `json:"execution_boundary"`
	// FallbackExecutor identifies the incumbent used if the candidate cannot complete.
	FallbackExecutor string `json:"fallback_executor,omitempty"`
	// SourceCommit identifies the source revision qualified by the promotion evidence.
	...
	// Caps maps each guarded resource dimension to its immutable limit.
	Caps map[string]int64 `json:"caps"`
	// Buckets contains the workload and structural scopes authorized for the candidate.
	Buckets []traversalPromotionBucket `json:"buckets"`
	// Evidence maps each required qualification role to its verified artifact.
	Evidence map[string]traversalPromotionEvidence `json:"evidence"`
}

// decodeTraversalPromotionManifest decodes one complete, duplicate-key-free promotion manifest.
...
// rejectDuplicateTraversalJSONValue recursively rejects duplicate keys in one JSON value.
...
// validate verifies that a policy's candidate, manifest, and rollback controls agree.
...
type topologyThreshold struct {
	// Name is the manifest key for this topology estimator threshold.
	Name string

	// Value is the immutable threshold value required by the estimator protocol.
	Value int64
}

// sortedTopologyThresholds turns an unordered threshold map into a deterministic sequence.
...
// lowerHexSHA256 reports whether value is a lowercase hexadecimal SHA-256 digest.
...
// productionCanaryExecutor reports whether executor is eligible for a production promotion manifest.
...
// effectiveTraversalPolicy selects the exact-query policy effective at the requested isolation level.
...
// hasStructuralTraversalPolicy reports whether a non-rollback policy needs structural classification.
...
// shortestPathExecutorRequiresStableSnapshot reports whether executor reads state that requires snapshot stability.
