...
type Translator struct {
	...
	translation Result
	// parameterSources maps generated parameter identifiers back to caller symbols.
...
func TranslateWithOptions(ctx context.Context, cypherQuery *cypher.RegularQuery, kindMapper pgsql.KindMapper, parameters map[string]any, graphID int32, options Options) (Result, error) {
	translation, _, err := translateWithOptions(ctx, cypherQuery, kindMapper, parameters, graphID, options, ToolOptions{}, false)
...
func TranslateWithOptionsAndParameterSources(ctx context.Context, cypherQuery *cypher.RegularQuery, kindMapper pgsql.KindMapper, parameters map[string]any, graphID int32, options Options) (Result, map[string]string, error) {
	return translateWithOptions(ctx, cypherQuery, kindMapper, parameters, graphID, options, ToolOptions{}, true)
}
...
func TranslateWithProductionOptions(ctx context.Context, cypherQuery *cypher.RegularQuery, kindMapper pgsql.KindMapper, parameters map[string]any, graphID int32, options ProductionOptions) (Result, error) {
	...
	translation, _, err := translateOptimized(ctx, optimizedPlan, kindMapper, parameters, graphID, toolOptions)
	return translation, err
}
...
func TranslateForTool(ctx context.Context, cypherQuery *cypher.RegularQuery, kindMapper pgsql.KindMapper, parameters map[string]any, graphID int32, options ToolOptions) (Result, error) {
	translation, _, err := translateWithOptions(ctx, cypherQuery, kindMapper, parameters, graphID, DefaultOptions(), options, false)
	return translation, err
}

func translateWithOptions(ctx context.Context, cypherQuery *cypher.RegularQuery, kindMapper pgsql.KindMapper, parameters map[string]any, graphID int32, options Options, toolOptions ToolOptions, borrowQuery bool) (Result, map[string]string, error) {
...
func translateOptimized(ctx context.Context, optimizedPlan optimize.Plan, kindMapper pgsql.KindMapper, parameters map[string]any, graphID int32, options ToolOptions) (Result, map[string]string, error) {
	...
	if options.ForceExpansionSearchStrategy != "" && len(translator.appliedExpansionSearchStrategies) == 0 {
		return Result{}, nil, fmt.Errorf("forced expansion-search strategy %q was selected but not emitted", options.ForceExpansionSearchStrategy)
	}
	if options.EnableExpansionOrientationTournament && len(translator.emittedExpansionSearchPolicies) == 0 {
		return Result{}, nil, fmt.Errorf("expansion orientation tournament was selected but not emitted")
	}
	if options.EnableExpansionOrientationShadow && len(translator.emittedExpansionSearchPolicies) == 0 {
		return Result{}, nil, fmt.Errorf("expansion orientation shadow was selected but not emitted")
	}
	if options.EnableExpansionSuffixReverseGuard && len(translator.emittedExpansionSearchPolicies) == 0 {
		return Result{}, nil, fmt.Errorf("expansion suffix reverse guard was selected but not emitted")
	}
	if options.EnableExpansionSuffixRouteComponent && len(translator.appliedExpansionSearchStrategies) == 0 {
		return Result{}, nil, fmt.Errorf("suffix route component was selected but not emitted")
	}
	if options.ForceShortestPathExecutor != "" && len(translator.appliedShortestPathExecutors) == 0 {
		return Result{}, nil, fmt.Errorf("forced shortest-path executor %q was selected but not emitted", options.ForceShortestPathExecutor)
	}
