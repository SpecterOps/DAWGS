...
func (s *Translator) translateMatch(match *cypher.Match) error {
	...
	// If there is no previous frame, then skip the `OPTIONAL MATCH` aggregation step
	if match.Optional && s.scope.CurrentFrame().Previous != nil {
		...
		// are corrected
		// TODO: Should this use a scope snapshot?
		for _, exported := range aggrFrame.Exported.Slice() {
			// TODO: What does !exists mean? We materialized an identifier that is not fully/correctly
